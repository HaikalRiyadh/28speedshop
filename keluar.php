...
<body class="sb-nav-fixed">
            ...
            <main>
                <div class="container-fluid px-4">
                    ...
                    if ($filterTglMulai && $filterTglAkhir) {
                        $whereClause = " AND DATE(k.tanggal) BETWEEN '" . $conn->real_escape_string($filterTglMulai) . "' AND '" . $conn->real_escape_string($filterTglAkhir) . "'";
                        ...
                        $whereClause = " AND DATE(k.tanggal) >= '" . $conn->real_escape_string($filterTglMulai) . "'";
                        $whereSummary = " WHERE DATE(tanggal) >= '" . $conn->real_escape_string($filterTglMulai) . "'";
                    } elseif ($filterTglAkhir) {
                        $whereClause = " AND DATE(k.tanggal) <= '" . $conn->real_escape_string($filterTglAkhir) . "'";
                        $whereSummary = " WHERE DATE(tanggal) <= '" . $conn->real_escape_string($filterTglAkhir) . "'";
                    }

                    $totalKeluar = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM keluar" . $whereSummary))['total'];
                    $totalQtyKeluar = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COALESCE(SUM(qty),0) as total FROM keluar" . $whereSummary))['total'];
                    ?>
                    <div class="row mb-4">
                            ...
                            <div class="d-flex align-items-center gap-2 flex-wrap">
                                <form method="get" class="d-flex align-items-center gap-2 flex-wrap">
                                    ...
                                    <?php if ($filterTglMulai || $filterTglAkhir): ?>
                                        <a href="keluar.php" class="btn btn-sm btn-outline-secondary"><i class="fas fa-times me-1"></i>Reset</a>
                                    <?php endif; ?>
                                </form>
                            ...
                            <div class="table-responsive">
                                <table id="datatablesSimple" class="table table-hover align-middle">
                                    ...
                                    <tbody>
                                        <?php
                                        $no = 1;
                                        $data = mysqli_query($conn, "SELECT k.*, s.namabarang FROM keluar k JOIN stock s ON k.idbarang = s.idbarang WHERE 1=1" . $whereClause . " ORDER BY k.tanggal DESC");
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <form method="post" class="d-inline form-delete"
                                                    ...
                                                    data-msg="Yakin ingin menghapus data keluar '<?php echo htmlspecialchars($row['namabarang'], ENT_QUOTES); ?>'? Stok akan dikembalikan.">
    ...
    <script>
        document.addEventListener('click', function(e) {
            var btn = e.target.closest('.btn-edit-keluar');
            if (btn) {
                document.getElementById('edit_idkeluar').value           = btn.dataset.id;
                document.getElementById('edit_keluar_idbarang').value    = btn.dataset.idbarang;
                document.getElementById('edit_keluar_qty_lama').value    = btn.dataset.qty;
                document.getElementById('edit_keluar_qty').value         = btn.dataset.qty;
                document.getElementById('edit_keluar_keterangan').value  = btn.dataset.keterangan;
            }
        });
